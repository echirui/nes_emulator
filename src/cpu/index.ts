...
export default class Cpu {
  ...
  getAddrOrDataWithAdditionalCycle(mode: AddressingMode): AddrOrDataAndAdditionalCycle {
    switch (mode) {
      ...
      case op.ADDR_MODE_ZERO_PAGE: {
        return {
          addrOrData: this.fetch(this.registers.PC),
          additionalCycle: 0,
        }
      }
      case op.ADDR_MODE_ZERO_PAGEX: {
      ...
      case op.ADDR_MODE_ABS: {
        return {
          addrOrData: (this.fetch(this.registers.PC, "Word")),
          additionalCycle: 0}
      }
      case op.ADDR_MODE_ABS_X: {
      ...
      case op.ADDR_MODE_PRE_IDX_INDIRECT: {
      ...
      case op.ADDR_MODE_POS_IDX_INDIRECT: {
      ...
      case op.ADDR_MODE_INDIRECT_ABS: {
      ...
      default: throw new Error(`Unkwon addressing mode ${mode} detected.`)
    }
  }
  ...
  execInstruction(baseName: string, addrOrData: Word, mode: AddressingMode) {
    ...
    switch (baseName) {
      ...
      case 'ADC': {
        ...
        const operated = data + this.registers.A + (this.registers.P.carry ? 1 : 0);
      ...
      case 'ISB': {
        const data = (this.read(addrOrData) + 1) & 0xFF;
        const operated = (~data & 0xFF) + this.registers.A + (this.registers.P.carry ? 1 : 0);
        const overflow = (!(((this.registers.A ^ data) & 0x80) != 0) && (((this.registers.A ^ operated) & 0x80)) != 0);
        this.registers.P.overflow = overflow;
        this.registers.P.carry = operated > 0xFF;
        this.registers.P.negative = !!(operated & 0x80);
      ...
      case 'RLA': {
        const data = (this.read(addrOrData) << 1) + (this.registers.P.carry ? 1 : 0);
      ...
      case 'RRA': {
        ...
        const operated = data + this.registers.A + (carry ? 1 : 0);
        const overflow = (!(((this.registers.A ^ data) & 0x80) != 0) && (((this.registers.A ^ operated) & 0x80)) != 0);
        this.registers.P.overflow = overflow;
        this.registers.P.negative = !!(operated & 0x80);
        this.registers.P.zero = !(operated & 0xFF);
