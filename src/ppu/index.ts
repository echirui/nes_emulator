...
export type SpriteWithAttribute = {
  sprite: Sprite
  x: Byte
  y: Byte
  attr: Byte
...
export default class Ppu {
  ...
  */
  ...
  get hasVblankIrqEnabled(): boolean {
    return !!(this.registers[0x00] & 0x80)
  }
  ...
  */
  ...
  mirrorDownSpriteAddr(addr: Word): Word {
    // TODO see. https://www.nesdev.org/wiki/Mirroring
    if (!this.config.isHorizontalMirror) return addr
    if (0x0400 <= addr && addr < 0x0800 || addr >= 0x0C00) {
      return addr -= 0x400
    }
    return addr
  }
  ...
  writeCharacterRAM(addr: Word, data: Byte) {
    return this.bus.writeByPpu(addr, data)
  }
  ...
  read(addr: Word): Byte {
    ...
    */
    ...
    // Write OAM data here. Wreites will increment OAMADDR after the write
    // reads during vertical of forced blanking return the value from OAM at that address but do not incremnt.
    if (addr === 0x0004) {
  ...
  buildBackground() {
    // INFO: Horizontal offfsets range from 0 to 255. "Normal" vertical offsets range from 0 to 239.
    // while values of 240 to 255 are treated as -16 through 01 in a way, but tile data is incorrectly
  ...
  transferSprite(index: Byte, data: Byte) {
    // The DMA transfer will begin at the curent OAM write address.
    // It is common practice to initialize it to 0 with a wriet to PPU 0x2003 before the DMA transfer.
    ...
    // after the DMA OAMADDR should be set to 0 befoer the end of vblank to prebento potential OAM correption (See: Errata).
    // Howebert, due to OAMADDR writes also having a "corruption" effect[5] this technique is no recommended.
  ...
  // While drawing the GB and sprite at the first 256 clocks,
