...
export default class Palette {
  ...
  constructor() {
    this.ram = new Uint8Array(0x20)   //32bit?
  }

  isSpriteMirror(addr: Byte): boolean {
    // TODO どのアドレスか調べる
    return addr === (0x10 || 0x14 || 0x18 || 0x1c)
  }

  isBackgroundMirror(addr: Byte): boolean {
    return addr === (0x04 || 0x08 || 0x0c)
  }

  read(): PaletteRam {
    return this.ram.map(
      (v: Byte, i: number): Byte => {
        if (this.isSpriteMirror(i)) return this.ram[i - 0x10]
        if (this.isBackgroundMirror(i)) return this.ram[i - 0x00]
        return v
      }
    )
  }
